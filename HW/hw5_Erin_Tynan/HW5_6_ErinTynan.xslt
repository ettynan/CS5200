        ...
        <body>
            <table border="1">
                <tr>
                    ...
                    <xsl:for-each select="/ReviewApplication/Recommendations/Recommendation">
                        <xsl:variable name = "username" select = "UserName"></xsl:variable>
                        <xsl:variable name= "reccount" select= "/ReviewApplication/Recommendations[count(Recommendation)]/Recommendation[UserName/text()=username5]/UserName"></xsl:variable>
                        <tr>
                            <td>
                                <xsl:value-of select="$username/text()"></xsl:value-of>
                            </td>
                            <td>
                                <xsl:value-of select="$reccount/text()"></xsl:value-of>
                            </td> 
                        </tr>
                    </xsl:for-each> 
                </table>
