...
public class TakeOutDao extends RestaurantsDao {
	...
	public TakeOut getTakeOutById(int restaurantId) throws SQLException {
        ...
        try {
            ...
            if(results.next()) {
                int restId = results.getInt("RestaurantId");
	...
	public List<TakeOut> getTakeOutByCompanyName(String companyName) throws SQLException {
		...
		String selectTOR =  "SELECT tor.RestaurantId, Name, Description, Menu, Hours, Active, Cuisine, Street1,"
        + "Street2, City, State, Zip, CompanyName, MaxWaitTime FROM TakeOutRestaurant AS tor JOIN Restaurants on tor.RestaurantId=Restaurants.RestaurantId "
		...
		try {
			...
			while(results.next()) {
				...
				String state = results.getString("State");
				int zip = results.getInt("Zip");
				String companyName17 = results.getString("CompanyName");
				int maxWaitTime = results.getInt("MaxWaitTime");
				
				TakeOut takeOut1 = new TakeOut(restaurantId, name, description, menu, hours, active, cuisine, street1, street2, city, state, zip, companyName17, maxWaitTime);
