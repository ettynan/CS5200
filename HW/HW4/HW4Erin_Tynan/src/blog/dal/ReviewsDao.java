...
//            
    ...
    public Reviews getReviewById(int reviewId) throws SQLException {
        ...
        try {
            ...
            if(results.next()) {
                int revId = results.getInt("ReviewId");
			    Date created = results.getDate("Created");
                String content = results.getString("Content");
                double rating = results.getDouble("Rating");
                String user = results.getString("UserName");                
                int rest = results.getInt("RestaurantId");
                Reviews review = new Reviews(revId, created, content, rating, user, rest);
                return review;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Reviews> getReviewsByRestaurantId(int restaurantId)
		...
		try {
			...
			while(results.next()) {
			    ...
			    int restaurantId2 = results.getInt("RestaurantId");
